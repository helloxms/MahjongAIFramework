...
class GameManager:
            ...
            if state["round_act_state"] != MJConstants.round_act_state.FINISHED:
                ...
                if state["cur_act"] == MJConstants.Action.TAKE:
                    ...
                    if check_action_result[0] == MJConstants.Action.TAKE:
                        print("==========> server: player{}, ok, i will drop this tile:{}".format(check_player_pos,
                                                                                                  check_action_result
                                                                                                  [1]))
                ...
                if check_action == MJConstants.Action.PLAY or check_action == MJConstants.Action.CHOW or check_action == MJConstants.Action.PONG:
                    choosed_player, choosed_action, choosed_tile = self.__get_choosed_player(state, check_player_pos,
                                                                                             check_action, False)
    ...
    def __get_choosed_player(self, state, player_pos, action, bInclude=False):
        ...
        for i in range(0, 4):
            if result_map[i][0] == MJConstants.Action.PONG:
                choosed_player = i
                choosed_action = MJConstants.Action.PONG
                return choosed_player, choosed_action, choosed_tile

        for i in range(0, 4):
            if result_map[i][0] == MJConstants.Action.CHOW and result_map[i][1] >= 0 and table.is_chow_pos(player_pos, i):
                choosed_player = i
        ...
        if choosed_action == MJConstants.Action.TAKE:
            print("all player pass, the next player{} take tile \n\n".format(choosed_player))
        return choosed_player, choosed_action, choosed_tile
